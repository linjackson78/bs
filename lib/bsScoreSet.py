    ...
    class _Player(object):
        ...
        def submitKill(self,showPoints=True):
            ...
            if self._multiKillCount == 1:
                ...
                score = 20
                name = bs.getResource('twoKillText')
                color=(0.1,1.0,0.0,1)
                scale = 1.0
                delay = 0
                ...
                score = 40
                name = bs.getResource('threeKillText')
                color=(1.0,0.7,0.0,1)
                scale = 1.1
                delay = 300
                ...
                score = 60
                name = bs.getResource('fourKillText')
                color=(1.0,1.0,0.0,1)
                scale = 1.2
                delay = 600
                ...
                score = 80
                name = bs.getResource('fiveKillText')
                color=(1.0,0.5,0.0,1)
                scale = 1.3
                delay = 900
                ...
                score = 100
                name = bs.getResource('multiKillText').replace('${COUNT}',str(self._multiKillCount))
                color=(1.0,0.5,0.0,1)
                scale = 1.3
                delay = 1000
                sound = self._scoreSet()._orchestraHitSound4
            ...
            def _apply(name,score,showPoints,color,scale,sound):
                ...
                if activity is not None:
                    bsUtils.PopupText((('+'+str(score)+' ') if showPoints else '')+name,
                                            color=color,
                                            scale=scale,
    ...
    def playerScored(self,player,basePoints=1,target=None, kill=False, victimPlayer=None,scale=1.0,color=None,title=None,screenMessage=True,display=True,importance=1,showPoints=True,bigMessage=False):
        ...
        if display and bigMessage:
            try:
                ...
                if activity:
                    activity.showZoomMessage(bs.getResource('nameScoresText').replace(u'${NAME}',player.getName()),color=bsUtils.getNormalizedColor(player.getTeam().color))
            except Exception,e:
                print 'Exception showing bigMessage',e

        # if we currently have a spaz, pop up a score over it
        if display and showPoints and p.getSpaz() is not None:
        ...
        try:
            if screenMessage and not kill:
                bs.screenMessage(bs.getResource('nameScoresText').replace('${NAME}',name),
    ...
    def playerLostSpaz(self, player, killed=False, killer=None):
        ...
        try:
            if killed and bs.getActivity().announcePlayerDeaths:
                ...
                if killer == player:
                    bs.screenMessage(bs.getResource('nameSuicideKidFriendlyText' if bsInternal._getSetting('Kid Friendly Mode') else 'nameSuicideText').replace('${NAME}',name),top=True,color=player.color,
                                     image=player.getIcon())
                elif killer is not None:
                    if killer.getTeam() == player.getTeam():
                        bs.screenMessage(bs.getResource('nameBetrayedText').replace(u'${NAME}',killer.getName()).replace(u'${VICTIM}',name),top=True,color=killer.color,
                                         image=killer.getIcon())
                    else:
                        bs.screenMessage(bs.getResource('nameKilledText').replace(u'${NAME}',killer.getName()).replace(u'${VICTIM}',name),top=True,color=killer.color,
                                         image=killer.getIcon())
                else:
                    bs.screenMessage(bs.getResource('nameDiedText').replace(u'${NAME}',name),top=True,color=player.color,
                                     image=player.getIcon())
        except Exception,e:
