...
class ConquestGame(bs.TeamGameActivity):
    ...
    def onBegin(self):
        ...
        # initial joiners didn't spawn due to no flags being owned yet; spawn them now
        for player in self.players:
            self.spawnPlayer(player)

    def _updateScores(self):

        for team in self.teams: team.gameData['flagsHeld'] = 0

        for flag in self._flags:
            try: flag.getTeam().gameData['flagsHeld'] += 1
            except Exception: pass

        for team in self.teams:
            # if a team finds themselves with no flags, cancel all outstanding spawn-timers
            ...
            if team.gameData['flagsHeld'] == len(self._flags):
                self.endGame()
            self._scoreBoard.setTeamValue(team,team.gameData['flagsHeld'],len(self._flags))

    def endGame(self):
        results = bs.TeamGameResults()
        for t in self.teams: results.setTeamScore(t,t.gameData['flagsHeld'])
    ...
    def _handleFlagPlayerCollide(self):
        ...
        except Exception: return # player may have left and his body hit the flag
        ...
        if flag.getTeam() is not player.getTeam():
            ...
            # respawn any players on this team that were in limbo due to the lack of a flag for their team
            for p in self.players:
                if p.getTeam() is flag.getTeam() and p.actor is not None and not p.isAlive() and p.gameData['respawnTimer'] is None:
                    self.spawnPlayer(p)
    ...
    def handleMessage(self,m):
        if isinstance(m,bs.PlayerSpazDeathMessage):
            ...
            if player.getTeam().gameData['flagsHeld'] > 0: self.respawnPlayer(m.spaz.getPlayer())
            else: player.gameData['respawnTimer'] = None
