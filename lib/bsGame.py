...
class Session(object):
        ...
        if self._useTeams:
            for i in range(len(teamColors)):
                team = bs.Team(teamID=self._nextTeamID,
                               name=GameActivity.getTeamNameLocalized(teamNames[i]),
                               color=teamColors[i])
    ...
    def onPlayerRequest(self,player):
        ...
        if bsUtils._gStressTestResetTimer is None:
            ...
            #         bs.screenMessage(bs.getResource('playerLimitReachedUnlockProText')
            #                          .replace('${PRO}',bs.getResource('store.bombSquadProNameText'))
            #                          .replace('${COUNT}',str(3)),
            #                          color=(0.8,0,0))
            #         return False
            ...
            if len(self.players) >= self._maxPlayers:
                bs.playSound(bs.getSound('error'))
                bs.screenMessage(bs.getResource('playerLimitReachedText').replace('${COUNT}',str(self._maxPlayers)),color=(0.8,0,0))
                return False
    ...
    def onPlayerLeave(self,player):
        ...
        if player in self.players:
            ...
            if team is not None:
                bs.screenMessage(bs.getResource('playerLeftText').replace('${PLAYER}',player.getName(full=True)))
        ...
        if activity is not None and activity._isJoiningActivity:
            if lobby.checkAllReady():
                ...
                if len(choosers) >= minPlayers:
                    ...
                    bs.screenMessage(bs.getResource('notEnoughPlayersText').replace('${COUNT}',str(minPlayers)),color=(1,1,0))
                    bs.playSound(bs.getSound('error'))
            else:
                return
    ...
    def _addChosenPlayer(self,chooser):
        ...
        if passToActivity:
            if not self._allowMidActivityJoins:
                ...
                with bs.Context(self):
                    bs.screenMessage(bs.getResource('playerDelayedJoinText').replace(u'${PLAYER}',player.getName(full=True)),color=(0,1,0))
        ...
        if self._useTeams:
            ...
            team = bs.Team(teamID=ourTeamID,
                           name=chooser.getPlayer().getName(),
...
class Activity(object):
                            ...
                            if True:
                                ...
                                if not g.paused and bsUI.uiGlobals['mainMenuWindow'] is None or not bsUI.uiGlobals['mainMenuWindow'].exists():
                                    ...
                                    with bs.Context('UI'):
                                        ...
                                        # bs.realTimer(1000,lambda: bsUI.ConfirmWindow(bs.getResource('continuePurchaseText').replace('${PRICE}','STUFF'),
                                        #                                              bs.WeakCall(self._continue),cancelButton=False))
                                    return
    ...
    def end(self,results=None,delay=0,force=False):
        ...
        # if results is a standard team-game-results, associate it with us so
        # it can grab our score prefs
        if isinstance(results,bs.TeamGameResults):
            results._setGame(self)
            
        # if we had a standard time-limit that hadnt expired, stop it so it doesnt tick annoyingly
...
class ScoreScreenActivity(Activity):
    ...
    def onBegin(self,customContinueMessage=None):
        ...
        if bs.getEnvironment()['interfaceType'] == 'desktop': s = bs.getResource('pressAnyKeyButtonText')
        else: s = bs.getResource('pressAnyButtonText')
...
class GameActivity(Activity):
    ...
    # @classmethod
    # def getScoreVersion(cls):
    #     """
    #     On games where local high scores are kept, this can be changed
    #     to prevent high scores from different versions of the game from
    #     intermingling with each other. Otherwise, if a game is made harder
    #     for example, it may be impossible to ever beat a local high score
    #     set in an older, easier version. This can be any string, but generally
    #     'A', 'B', 'C', etc are used.
    #     """
    #     return ""
    ...
    @classmethod
    def getNameLocalized(cls,settings=None):
        """
        Return a translated name for this game type.
        Subclasses should override getName(); not this.
        """
        name = cls.getName()
        name = bs.translate('gameNames',name)
        ...
        if settings is not None:
            if 'Solo Mode' in settings and settings['Solo Mode']:
                name = bs.getResource('soloNameFilterText').replace('${NAME}',name)
            if 'Epic Mode' in settings and settings['Epic Mode']:
                name = bs.getResource('epicNameFilterText').replace('${NAME}',name)
        
        return name

    @classmethod
    def getTeamNameLocalized(cls,name):
        """
        Given a team name, returns a translated version of it.
        """
        #try: translated = bs.getResource('translations.teamNames')[name]
        return bs.translate('teamNames',name)
    ...
    @classmethod
    def getDescriptionLocalized(cls,sessionType):
        """
        ...
        return bs.translate('gameDescriptions',description)
    ...
    def showScoreBoardInfo(self):
        ...
        # try: translation = bs.getResource('translations.gameDescriptions')[sbDesc[0]]
        # except Exception: translation = None
        # if translation is None: translation = sbDesc[0]
        translation = bs.translate('gameDescriptions',sbDesc[0])
        for i in range(len(sbDesc)-1):
            translation = translation.replace('${ARG'+str(i+1)+'}',str(sbDesc[i+1]))
        sbDesc = translation
    ...
    def showInfo(self):
        ...
        #try: translation = bs.getResource('translations.gameDescriptions')[desc[0]]
        # except Exception: translation = None
        # if translation is None: translation = desc[0]
        translation = bs.translate('gameDescriptions',desc[0])
        for i in range(len(desc)-1):
            translation = translation.replace('${ARG'+str(i+1)+'}',str(desc[i+1]))

        # do some standard filters (epic mode, etc)
        if ('Epic Mode' in self.settings and self.settings['Epic Mode']):
            translation = bs.getResource('epicDescriptionFilterText').replace('${DESCRIPTION}',translation)
    ...
    def _showTip(self):
        ...
        if len(self.tips) > 0:
            ...
            tipTitle = bs.getResource('tipText')+':'
            icon = None
            sound = None
            if type(tip) == dict:
            ...
            tipScale = 0.8
            tipTitleScale = 1.2
            tipWidthRaw = bs.getStringWidth(tip)
            if tipWidthRaw > 950: tipScale *= (950.0/tipWidthRaw)
            tipWidth = tipWidthRaw*tipScale
    ...
    def _standardTimeLimitGetStr(self):
        value = self._standardTimeLimitTime
        return bsUtils.getTimeString(value*1000,centi=False)

    def _tournamentTimeLimitGetStr(self):
        value = self._tournamentTimeLimitTime
        return bsUtils.getTimeString(value*1000,centi=False)
    ...
    def setupStandardTimeLimit(self,duration):
        ...
        self._standardTimeLimitText = bs.NodeActor(bs.newNode('text',
                                                              attrs={'vAttach':'top',
                                                                     ...
                                                                     'flatness':1.0,
                                                                     'scale':0.9,
                                                                     'text':self._standardTimeLimitGetStr()}))
        ...
        if self._standardTimeLimitTime <= 0:
            ...
            n = bs.newNode('text',
                           attrs={'vAttach':'top','hAttach':'center',
                                  ...
                                  'position':(0,-90),'scale':1.2,'text':bs.getResource('timeExpiredText')})
            bs.playSound(bs.getSound('refWhistle'))
            bsUtils.animate(n,"scale",{0:0.0, 100:1.4, 150:1.2})

        self._standardTimeLimitText.node.text = self._standardTimeLimitGetStr()
    ...
    def _setupTournamentTimeLimit(self,duration):
        ...
        self._tournamentTimeLimitTitleText = bs.NodeActor(bs.newNode('text',
                                                              attrs={'vAttach':'bottom',
                                                                     ...
                                                                     'flatness':1.0,
                                                                     'scale':0.5,
                                                                     'text':bs.getResource('tournamentText')}))
        self._tournamentTimeLimitText = bs.NodeActor(bs.newNode('text',
                                                              attrs={'vAttach':'bottom',
                                                                     ...
                                                                     'flatness':1.0,
                                                                     'scale':0.9,
                                                                     'text':self._tournamentTimeLimitGetStr()}))
    def _tournamentTimeLimitTick(self):
        ...
        if self._tournamentTimeLimitTime <= 10:
            if self._tournamentTimeLimitTime == 10:
                ...
                #self._tournamentTimeLimitTitleText.node.maxWidth = 100
                self._tournamentTimeLimitText.node.position = (80,60)
                #self._tournamentTimeLimitText.node.maxWidth = 100
        ...
        if self._tournamentTimeLimitTime <= 0:
            ...
            n = bs.newNode('text',
                           attrs={'vAttach':'top','hAttach':'center',
                                  'hAlign':'center','color':(1,0.7,0,1),
                                  'position':(0,-200),'scale':1.6,'text':bs.getResource('tournamentTimeExpiredText',fallback='timeExpiredText')})
            bs.playSound(bs.getSound('refWhistle'))
            bsUtils.animate(n,"scale",{0:0.0, 100:1.4, 150:1.2})

        self._tournamentTimeLimitText.node.text = self._tournamentTimeLimitGetStr()
