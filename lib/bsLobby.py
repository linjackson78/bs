...
class Chooser(object):
    ...
    def __init__(self,vPos,player,lobby):
        ...
        if gRandomCharIndexOffset is None:
            ...
            gRandomCharIndexOffset = random.randrange(100)
            # gRandomCharIndexOffset = self.getLobby().characterNames.index('Spaz')-inputDeviceID
            # if gRandomCharIndexOffset < 0: gRandomCharIndexOffset += len(self.getLobby().characterNames)
            
        self._randomCharacterIndex = (inputDeviceID+gRandomCharIndexOffset)%len(self.getLobby().characterNames)
        ...
        try:
            ...
            name = bs.uni(inputDevice.getName())
        ...
        # well, that didn't work.. pick __account__, _random, or some other random profile..
        except Exception:
        ...
        self._player.setName(bs.getResource('choosingPlayerText'))
    ...
    def updateFromPlayerProfile(self):
        ...
        try:
            ...
            self.characterIndex = self.getLobby().characterNames.index(character)
    ...
    def reloadProfiles(self):
        ...
        if isRemote:
            try:
                self.profiles = ast.literal_eval(inputDevice._getProfilesStr())
            except Exception:
                self.profiles = {}
        else:
    ...
    def getCharacterName(self):
        return self.getLobby().characterNames[self.characterIndex]
    ...
    def _getName(self,full=False):
        ...
        if name == '_random':
            ...
            name = bs.getResource('createEditPlayerText',fallback='editProfileWindow.titleNewText')
        else:
    ...
    def _setReady(self,ready):
        ...
        if not ready:
            ...
            self._player.setName(self._getName(),self._getName(full=True))
            self.ready = True
            self._updateText()
        ...
        if isinstance(m,ChangeMessage):
            ...
            if m.what == 'team':
                ...
                if len(self.profileNames) == 1:
                    ...
                    # global gLastWarnTime
                    # if bs.getRealTime() - gLastWarnTime > 5000:
                    #     gLastWarnTime = bs.getRealTime()
                    #     bs.screenMessage(bs.getResource('noProfilesErrorText').replace('${NAME}',self._getName()))
                else:
                ...
                self.characterIndex = (self.characterIndex + m.value) % len(self.getLobby().characterNames)
                self._updateText()
                self._updateIcon()
    ...
    def _updateText(self):
        ...
        if self.ready:
            ...
            text = self._player.getName(full=True)
            text = text + ' ('+bs.getResource('readyText')+')'
        else:
    ...
    def getColor(self):
        ...
        if self.getLobby()._useTeamColors:
            return self.getLobby()._teams[self._selectedTeamIndex]().color
        else:
            return self._color
    ...
    def _updateIcon(self):
        ...
        texName = bsSpaz.appearances[self.getLobby().characterNames[self.characterIndex]].iconTexture
        tintTexName = bsSpaz.appearances[self.getLobby().characterNames[self.characterIndex]].iconMaskTexture
...
class Lobby(object):
    ...
    class JoinInfo(object):
        ...
        def __init__(self,lobby):
            ...
            if kb is not None:
                ...
                pressToPunch = bs.getResource('orText').replace('${A}','\''+punchKey+'\'').replace('${B}',pressToPunch)
                bombKey = kb.getButtonName(bsUI.getControllerValue(kb,'buttonBomb'))
                pressToBomb = bs.getResource('orText').replace('${A}','\''+bombKey+'\'').replace('${B}',pressToBomb)
                joinStr = bs.getResource('pressPunchToJoinText')+'  < '+pressToPunch+' >'
            else:
                joinStr = bs.getResource('pressAnyButtonToJoinText')
            ...
            self._text = bs.NodeActor(bs.newNode('text',
                                                 attrs={'position':(0,-40),
                                                        ...
                                                        #'text':bs.getResource('waitingForPlayersText',fallback='waitingForLocalPlayersText')}))
                                                        'text':joinStr}))
                
            self._messages = (([bs.getResource('pressToSelectTeamText').replace('${BUTTONS}',bs.getSpecialChar('leftArrow')+' '+bs.getSpecialChar('rightArrow'))] if canSwitchTeams else [])
                              + [bs.getResource('pressToSelectProfileText').replace('${BUTTONS}',bs.getSpecialChar('upArrow')+' '+bs.getSpecialChar('downArrow'))]
                              + [bs.getResource('pressToOverrideCharacterText').replace('${BUTTONS}',bs.getResource('bombBoldText'))+' < '+pressToBomb+' >']
                              + [joinStr])
    ...
    def reloadProfiles(self):
        ...
        self.characterNames = bsSpaz.getAppearances()
        self.characterNames.sort(key=lambda x:x.lower())
