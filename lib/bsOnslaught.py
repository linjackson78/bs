...
class OnslaughtGame(bs.CoopGameActivity):
    ...
    def onTransitionIn(self):
        ...
        self._scoreBoard = bs.ScoreBoard(label=bs.getResource('scoreText'),scoreSplit=0.5)
    ...
    def doEnd(self,outcome,delay=0):
        ...
        if self._wave >= 2:
            ...
            score = None
            failMessage = bs.getResource('reachWave2Text')
    ...
    def _updateWaves(self):
        ...
        if self._canEndWave and not self._bots.haveLivingBots() and not self._gameOver:
            ...
            if won:
                ...
                self.showZoomMessage(bs.getResource('victoryText'),scale=1.0,duration=4000)
    ...
    def _awardCompletionBonus(self):
        ...
        for player in self.players:
            try:
                if player.isAlive():
                    self.scoreSet.playerScored(player,int(100/len(self.initialPlayerInfo)),scale=1.4,color=(0.6,0.6,1.0,1.0),title=bs.getResource('completionBonusText'),screenMessage=False)
            except Exception,e:
                print 'EXC in _awardCompletionBonus',e
        
    def _awardTimeBonus(self,bonus):
        bs.playSound(self._cashRegisterSound)
        bs.PopupText('+'+str(bonus)+' '+bs.getResource('timeBonusText'),
                           color=(1,1,0.5,1),
                           scale=1.0,
    ...
    def _awardFlawlessBonus(self,player):
        ...
        try:
            if player.isAlive():
                self.scoreSet.playerScored(player,self._flawlessBonus,scale=1.2,color=(0.6,1.0,0.6,1.0),title=bs.getResource('flawlessWaveText'),screenMessage=False)
        except Exception,e:
        ...
        if not any(player.isAlive() for player in self.teams[0].players):
            ...
            for player in self.players:
                if not player.isAlive() and (self._preset in ['endless','endlessTournament'] or (player.gameData['respawnWave'] <= len(self._waves))):
                    t += bs.getResource('onslaughtRespawnText').replace(u'${PLAYER}',player.getName()).replace(u'${WAVE}',unicode(player.gameData['respawnWave']))+'\n'
    ...
    def _startNextWave(self):
        ...
        self.showZoomMessage(bs.getResource('waveText')+' '+str(self._wave),
        ...
        self._timeBonusText = bs.NodeActor(bs.newNode('text',
                                                      attrs={'vAttach':'top',
                                                             ...
                                                             'text':bs.getResource('timeBonusText')+': '+str(self._timeBonus)}))
        ...
        self._waveText = bs.NodeActor(bs.newNode('text',
                                                 attrs={'vAttach':'top',
                                                        ...
                                                        'text':bs.getResource('waveText')+' '+str(self._wave)+('' if self._preset in ['endless','endlessTournament'] else ('/'+str(len(self._waves))))}))
    ...
    def _updateTimeBonus(self):
        ...
        if self._timeBonus > 0 and self._timeBonusText is not None:
            self._timeBonusText.node.text = bs.getResource('timeBonusText')+': '+str(self._timeBonus)
