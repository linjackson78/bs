...
class ChosenOneGame(bs.TeamGameActivity):
    ...
    def getSettings(cls,sessionType):
        return [("Chosen One Time",{'minValue':10,'default':30,'increment':10}),
    ...
    def _getChosenOnePlayer(self):
        if self._chosenOnePlayer is not None and self._chosenOnePlayer.exists(): return self._chosenOnePlayer
        else: return None
    ...
    def _tick(self):
        ...
        # player is None
            ...
            # this shouldnt happen, but just in case..
            # (chosen-one player ceasing to exist should trigger onPlayerLeave which resets chosen-one)
    ...
    def endGame(self):
        ...
        for team in self.teams: results.setTeamScore(team,self.settings['Chosen One Time'] - team.gameData['timeRemaining'])
    ...
    def handleMessage(self,m):
        if isinstance(m,bs.PlayerSpazDeathMessage):
            ...
            if player is self._getChosenOnePlayer():
                ...
                self._setChosenOnePlayer(None if (killerPlayer is None or killerPlayer is player or not killerPlayer.isAlive()) else killerPlayer)
            self.respawnPlayer(player)
        else: bs.TeamGameActivity.handleMessage(self,m)

    def _updateScoreBoard(self):
        for team in self.teams:
            self._scoreBoard.setTeamValue(team,team.gameData['timeRemaining'],self.settings['Chosen One Time'],countdown=True)
