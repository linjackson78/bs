...
class Achievement(object):
    ...
    def getNameLocalized(self):
        val = bs.uni(bs.getResource('achievements')[self._name]['name'])
        # name might have ${LEVEL} in it; in that case we substitute
        # the translated level name
        try:
            if '${LEVEL}' in val:
                import bsCoopGame
                campaignName,campaignLevel = self._level.split(':')
                n = bsCoopGame.getCampaign(campaignName).getLevel(campaignLevel).getDisplayNameLocalized()
                val = val.replace('${LEVEL}',n)
        except Exception,e:
            bs.printException()
        return val

    def getDescriptionLocalized(self):
        ach = bs.getResource('achievements')[self._name]
        return bs.uni(ach['description'] if 'description' in ach else ach['descriptionFull'])

    def getDescriptionCompleteLocalized(self):
        ach = bs.getResource('achievements')[self._name]
        return bs.uni(ach['descriptionComplete'] if 'descriptionComplete' in ach else ach['descriptionFullComplete'])
    
    def getDescriptionFullLocalized(self):
        try: levelName = bs.translate('coopLevelNames',gAchLevelNames[self._name])
        except Exception: levelName = '?'
        return bs.uni(bs.getResource('achievements')[self._name]['descriptionFull']).replace('${LEVEL}',levelName)

    def getDescriptionFullCompleteLocalized(self):
        try: levelName = bs.translate('coopLevelNames',gAchLevelNames[self._name])
        except Exception: levelName = '?'
        return bs.uni(bs.getResource('achievements')[self._name]['descriptionFullComplete']).replace('${LEVEL}',levelName)
    ...
    def createDisplay(self,x,y,delay,outDelay=None,color=None,style='postGame'):
        ...
        if inGameColors:
            ...
            txt = self.getNameLocalized()
            txtS = 0.85
            txtMaxW = 300
            #txtW = min(txtMaxW,bs.getStringWidth(txt)*txtS)
            ...
            txt2S = 0.62
            txt2MaxW = 400
            #txt2W = min(txtMaxW,bs.getStringWidth(txt2)*txt2S)
            objs.append(bsUtils.Text(self.getDescriptionFullLocalized() if inMainMenu else self.getDescriptionLocalized(),
                                     hostOnly=True,
                                     maxWidth=txt2MaxW,
                                     position=(x,y-14),
                                     transition='fadeIn',
            ...
            if hmo:
                t = bsUtils.Text(bs.getResource('difficultyHardOnlyText'),
                                 hostOnly=True,
            ...
            objs.append(bsUtils.Text(self.getNameLocalized(),
                                     hostOnly=True,
                                     maxWidth=300,
                                     position=(x,y+2),
                                     transition='inRight',
                                     ...
                                     transitionDelay=delay+50,
                                     transitionOutDelay=None).autoRetain())
            objs.append(bsUtils.Text(self.getDescriptionCompleteLocalized() if complete else self.getDescriptionLocalized(),
                                     hostOnly=True,
                                     maxWidth=400,
                                     position=(x,y-14),
                                     transition='inRight',
    ...
    def showCompletionBanner(self,sound=True):
        ...
        obj = bsUtils.Text(bs.getResource('achievementText')+':',
        ...
        obj = bsUtils.Text(self.getNameLocalized(),
        ...
        obj = bsUtils.Text(self.getDescriptionCompleteLocalized(),
