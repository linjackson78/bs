...
class MainMenuActivity(bs.Activity):
    ...
    def onTransitionIn(self):
        ...
        foo = bs.getResource('hostIsNavigatingMenusText').replace(u'${HOST}',bsInternal._getAccountDisplayString())
        self._hostIsNavigatingText = bs.NodeActor(bs.newNode('text',
                                                      attrs={'text':bs.getResource('hostIsNavigatingMenusText').replace('${HOST}',bsInternal._getAccountDisplayString()),
                                                             'clientOnly':True,
                                                             'position':(0,-200),
                                                             'flatness':1.0,
                                                             'hAlign':'center'}))
        ...
        class News(object):
                ...
                if time.time()-gLastNewsFetchTime > 600.0:
                    ...
                    if self._text is not None:
                        ...
                        if val == '__ACH__':
                            ...
                            bsUtils.Text(bs.getResource('nextAchievementsText'),
            ...
            def _gotNews(self,data):
                ...
                with bs.Context(activity):
                    ...
                    try:
                        ...
                        if env['testBuild']:
                            if 'latestTestBuild' in data and 'latestTestVersion' in data and data['latestTestBuild'] > env['buildNumber']:
                                def foo():
                                    bs.screenMessage(bs.getResource('newTestBuildAvailableText').replace('${VERSION}',str(data['latestTestVersion'])).replace('${BUILD}',str(data['latestTestBuild'])).replace('${ADDRESS}','files.froemling.net'),(0.7,1,0))
                            ...
                            if newestVersion > ourVersion:
                                def foo():
                                    #bs.screenMessage("A newer version of BombSquad is available!  ("+data['latestVersion']+')',(0.7,1,0))
                                    bs.screenMessage(bs.getResource('newVersionAvailableText').replace('${VERSION}',data['latestVersion']).replace('${APP_NAME}',bs.getResource('titleText')),(0.7,1,0))
                                    bs.playSound(bs.getSound('ding'))
                                bs.gameTimer(2000,foo)
                    except Exception,e:
                    ...
                    sc = 1.2 if (bs.getEnvironment()['interfaceType'] == 'phone' or bs.getEnvironment()['vrMode']) else 0.8
    ...
    def _update(self):
        ...
        # if language has changed, recreate our logo
        l = bs.getLanguage()
        if l != self._language:
            self._language = l
            ...
            self.version = bs.NodeActor(bs.newNode('text',
                                                   attrs={'vAttach':'bottom',
                                                          #'hAttach':'center' if vrMode else 'right',
                                                          'hAttach':'right',
                                                          'hAlign':'right',
                                                          'flatness':1.0,
                                                          'vrDepth':-10,
                                                          ...
                                                          'scale':0.9 if (interfaceType == 'phone' or vrMode) else 0.7,
                                                          'position':(-260,10) if vrMode else (-10,10),
                                                          'text':env['version']+(' ('+str(env['buildNumber'])+')' if (env['debugBuild'] or env['testBuild'] or forceShowBuildNumber) else '')+(' '+'('+bs.getResource('debugText')+')' if env['debugBuild'] else '')}))
            if not gDidInitialTransition:
            ...
            if env['testBuild'] and not env['kioskMode']:
                self.betaInfo = bs.NodeActor(bs.newNode('text',
                                                        attrs={'vAttach':'center',
                                                               ...
                                                               'text':bs.getResource('testBuildText',fallback='publicBetaText')}))
                if not gDidInitialTransition:
...
class SplashScreenActivity(bs.Activity):
    ...
    def onTransitionIn(self):
        ...
        #bs.gameTimer(self._part1Duration, self._startPart2)
    ...
    def _startPart2(self):
        ...
        t = bs.translate('tips','If you are short on controllers, install the \'${REMOTE_APP_NAME}\' app\n'
