...
def getLocalizedMapName(name):
    #try: translated = bs.getResource('translations.mapsNames')[name]
    # try: translated = 
    # except Exception: translated = None
    # if translated is None: translated = name
    return bs.translate('mapsNames',name)
...
class AlwaysLandMap(Map):
    ...
    def __init__(self):
        ...
        t = bs.newNode('text',
                       attrs={'text':bs.getResource('pressJumpToFlyText'),
                              'scale':1.2,
                              'maxWidth':800,
                              'position':(0,200),
                              'shadow':0.5,
